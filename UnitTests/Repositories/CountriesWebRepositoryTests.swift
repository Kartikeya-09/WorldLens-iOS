...
@Suite(.serialized) final class CountriesWebRepositoryTests {
    ...
    @Test func countryDetailsSuccess() async throws {
        ...
        try mock(.countryDetails(alpha3Code: country.alpha3Code), result: .success(value))
        let response = try await sut.details(country: country.dbModel())
        #expect(response == value)
    }

    @Test func countryDetailsHttpError() async throws {
        let countries = await ApiModel.Country.mockedData
        let country = countries[0]
        let value = ApiModel.CountryDetails(capital: "", currencies: [], borders: [])
        try mock(.countryDetails(alpha3Code: country.alpha3Code),
                 result: .success(value),
                 httpCode: 404)
        await #expect(throws: APIError.httpCode(404)) {
