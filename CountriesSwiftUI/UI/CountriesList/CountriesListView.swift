...
struct CountriesList: View {
    ...
    @State private(set) var countriesLoadState: Loadable<Void>
    ...
    init(state: Loadable<Void> = .notRequested) {
        self._countriesLoadState = .init(initialValue: state)
    }

    var body: some View {
        NavigationStack(path: $navigationPath) {
            content
                // View reads persisted countries via Query; Interactor refresh updates storage in background.
...
private extension CountriesList {
    ...
    private func loadCountriesList(forceReload: Bool) {
        ...
        // MVVM flow: View intent -> Interactor use case -> repositories mutate data sources.
        $countriesLoadState.load {
