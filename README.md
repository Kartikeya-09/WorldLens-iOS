...
The details request uses the alpha-code endpoint (`/alpha/{code}`) for stable lookups.
...
MVVM-style flow in this project:
* View emits intent (`onAppear`, pull-to-refresh, tap handlers)
* Interactor executes a use case (orchestrates repositories)
* Repository reads/writes network and persistence
* View automatically re-renders when `Loadable` or queried data changes
